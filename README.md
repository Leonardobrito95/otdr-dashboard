...
> Dashboard interno que monitora mais de 17.000 ONUs de uma rede de fibra óptica em tempo real, cruzando a API da SmartOLT com o billing (IXC) e um histórico próprio em PostgreSQL, com alertas autônomos quando a qualidade do sinal degrada.
...
Numa ISP, a saúde da rede de fibra vive na plataforma da OLT (SmartOLT), mas os dados de cliente e contrato vivem no ERP (IXC). Para saber quais clientes estão com sinal crítico na OLT de Águas Claras, alguém precisava cruzar dois sistemas na mão. E quando o sinal de uma ONU caía, ninguém sabia até o cliente ligar.

## 💡 A solução

Um dashboard que unifica as duas fontes e adiciona uma camada de histórico e alerta:

- **Visão em tempo real** das mais de 17.000 ONUs: nível de sinal (dBm), status (OK, Crítico ou Fora de Operação), LOS e falhas de energia.
- **Cruzamento OLT com IXC:** cada ONU é associada ao cliente e contrato, com filtro por OLT, região e faixa de sinal.
- **Histórico em PostgreSQL:** snapshots periódicos permitem ver a degradação ao longo do tempo, não só o instante atual.
- **Alertas autônomos por e-mail:** quando uma OLT ultrapassa um percentual de ONUs críticas, dispara alerta com cooldown anti-spam.

## 🧩 Componentes

| Arquivo | Papel |
|---|---|
| `dashboard/app.py` | Aplicação Flask, API e telas (KPIs, mapa, OS, histórico) |
...
| Fontes | **SmartOLT API** (REST) e **MySQL** (IXC) |
| Histórico | **PostgreSQL** |
| Alertas | SMTP (e-mail) com cooldown |
| Coleta | Threading com polling configurável |

## 📊 O que o dashboard mostra

- **KPIs:** total de ONUs, percentual crítico, fora de operação, por OLT.
